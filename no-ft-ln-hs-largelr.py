...
if __name__ == '__main__':
    ...
    fh = logging.FileHandler('no_ft_ln_hs_largelr.log')
    ...
    if CONTINUE:
        import glob
        param_values = glob.glob('no_ft_ln_hs_largelr_param_values_*.pkl')
        max_epoch = max(map(lambda x: int(x[len('no_ft_ln_hs_largelr_param_values_'):-len('.pkl')]), param_values))
        ...
        param_values_file = 'no_ft_ln_hs_largelr_param_values_{}.pkl'.format(max_epoch)
        ...
        [total_loss_values, order_embedding_loss_values,
         recurrent_norm_values]= pickle.load(open('no_ft_ln_hs_largelr_training_losses.pkl', 'rb'))

        [validation_total_loss_values, validation_order_embedding_loss_values] = pickle.load(
                                      open('no_ft_ln_hs_largelr_validation_losses.pkl', 'rb'))
    ...
    for e in xrange(START, NUM_EPOCHS + 1):
        ...
        pickle.dump({'resnet':lasagne.layers.get_all_param_values(resnet['pool5']),
                     ...
                     'lr':RECURR_LR.get_value()},
                      open('no_ft_ln_hs_largelr_param_values_{}.pkl'.format(e), 'wb'), protocol=-1)
        logger.info("Saving loss values for epoch {}".format(e))
        pickle.dump([total_loss_values, order_embedding_loss_values,
                     recurrent_norm_values],
                     open('no_ft_ln_hs_largelr_training_losses.pkl', 'wb'), protocol=-1)
        ...
        pickle.dump([validation_total_loss_values, validation_order_embedding_loss_values],
                    open('no_ft_ln_hs_largelr_validation_losses.pkl', 'wb'), protocol=-1)
