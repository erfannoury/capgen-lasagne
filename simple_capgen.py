...
if __name__ == '__main__':
    ...
    TOTAL_GRAD_CLIP = 64
    TOTAL_MAX_NORM = 128
    RESNET_ADAM_LR = theano.shared(np.float32(0.001 * 2), 'resnet_adam lr')
    RECURR_ADAM_LR = theano.shared(np.float32(0.01 * 2), 'recurrent adam lr')
    ...
    cap_out_var = T.itensor3('cap_out') # batch size, seq len, word one hot
    ...
    cap_out_var = cap_out_var.reshape((-1, cap_out_var.shape[1]), ndim=2)
